...
%These are two potential issues caused by the public nature of the work being done for the course. A few students, such as P7 and P11, suggested that it would have made little difference had the course repository been private, meaning only those in the class could see it.
...
Another point that students felt would hinder GitHub as a learning tool was the lack of integration with the University of Victoria login, similar to that which LMS such as CourseSpaces have. This has a few consequences: %different user/email, no \'official\' UVic stuff like grades
...
A few students described one drawback behind using GitHub for education - it's just not built for it. Those that mentioned this particular drawback acknowledged that there's workarounds for many of the tasks needed, but that GitHub certainly struggles to meet some educational needs such as Gradebooks and proper Assignment Submission. %quotes

%But I believe that mainly as GH is um.. for projects hostings, it's not specifically for learning, like if you see CS or some other dedicated tools. Like we can surely use GH, but I guess they are not directly, or implied that we can use them. Like for example, if we want to initiate some discussions, there's a workaround way to use issues, but I think issues are like for projects and stuff. [P8]

%Well.. at least for CS, you have... uh, but see yeah that's getting back into the whole school thing, and not the GH thing. Because at least with CS, you can access your grades and access your schedule that's coming up, and you can access your entire transcript. But that needs to stay in the school domain, not in the public GH. Yeah. [P10]

%Yeah cause right now it definitely feels like we're using a tool that's geared for something else and trying to throw education on top of it right? So if you could kind of integrate them better into another tool or maybe a plugin for GH if they ever did something like that. [P14]

%Like deliverables.; Like grades.; Announcements that you'd actually get by email, not just code, commit changes. [P14]
...
%this might be workflow related
...
%\textit{``I initially wanted to post all the issues and our conversation so we could account for who's doing what. But they basically didn't respond to them at all, so I moved to email. They responded to that a couple of days later so that was nice, but yeah.; Um... one of them uses GH semi-regularly, the other one doesn't. So I think they prefer email.''} [P2]

%\textit{``so basically we had to show it to our TA Nico that we have done it, and he used to mark it in a piece of paper. So putting it here was not really necessary right?; Mmmm it was not really clear. I think if uh you go into lab 5, it will say that basically you can help someone, uh... helping 3 other projects make progress, and having someone else help you make progress with yours. But like uh, like there was a little bit of confusion regarding this as well, so people didn't really help each other.''} [P5]

%\textit{``I don't see any benefit that GH has offered that we wouldn't have had in CS. All it appears to me is it's a place where she's.. it's a file repo for what it's being used, and we already have that. So I'm trying to see where the additional benefit, other than it being semi-confusing as to when I'm trying to look for a lab or an assignment and I now have two places to check instead of one.; I see no benefit to it being GH over just in CS or Connex or whatever they wanted to use.''} [P7]

%\textit{``Um, yeah if there was a way to uh.. collaborate on the material, that would be useful. Or maybe we can use it as uh.. even though there could be uh.. a part where you could do your submissions on there, that would be neat. But in this class, every one of our labs so far has been demo to the lab TA, so.. Nothing's going back to GH.; Right, maybe if there was a way to.. if we were submitting things to it. Maybe that would be helpful. I can see how it could be useful, it's just that in our usage it's not really adding anything to the experience.''} [P7]
